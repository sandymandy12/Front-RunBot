...
const {NETWORK, UNISWAP_ROUTER_ADDRESS, UNISWAP_FACTORY_ADDRESS, UNISWAP_ROUTER_ABI, UNISWAP_FACTORY_ABI, UNISWAP_POOL_ABI, HTTP_PROVIDER_LINK, WEBSOCKET_PROVIDER_LINK, HTTP_PROVIDER_LINK_TEST} = require('../constants/constants.js');
const {setBotAddress, getBotAddress, FRONT_BOT_ADDRESS, botABI} = require('./bot.js');

const {PRIVATE_KEY, TOKEN_ADDRESS, AMOUNT, LEVEL} = require('../constants/.env.js');
...
const handleTransaction = async function (transaction, out_token_address, user_wallet, amount, level) {
...
const approve = async function (gasPrice, outputtoken, out_token_address, user_wallet){
    var allowance = await out_token_info.token_contract.methods.allowance(user_wallet.address, UNISWAP_ROUTER_ADDRESS).call();
    
    console.log('Current Allowance: ', allowance/(10**out_token_info.decimals));
...
const triggersFrontRun = async function(transaction, out_token_address, amount, level) {
...
const swap = async function(gasPrice, gasLimit, outputtoken, outputeth, trade, out_token_address, user_wallet, transaction) {
...
const getCurrentGasPrices = async function getCurrentGasPrices() {
...
const isPending = async function(transactionHash) {
    return await web3.eth.getTransactionReceipt(transactionHash) == null;
}

const updatePoolInfo = async function() {
...
const getPoolInfo = async function(input_token_address, out_token_address, level){
...
const getTokenInfo = async function(tokenAddr, token_abi_ask, user_wallet) {
    //get token abi 
...
const preparedAttack = async function(out_token_address, user_wallet, address, amount, level)
{
//    try {
    ...
    log_str = 'wallet address:\t' + user_wallet;
    console.log(log_str.white);
...
const setFrontBot = async function(address, user_wallet){
